...
app.use(express.static("UniRoomAdmin/public/stylesheets"));
app.use(express.static("UniRoomAdmin/public/javascripts"));
...
app.post("/login", (req, res) => {
  ...
  if (req.cookies.token == undefined) {
    ...
    let query = "SELECT * FROM aiuroom.person WHERE UniversityEmail = '" + name + "'";
    con.query(query, (err, result) => {
      ...
      if (result[0].Password == pass) {
        ...
        transporter.sendMail(
          SignInEmail(result[0].name, result[0].ID, result[0].email),
          (err, info) => {
            if (err) throw err;
    ...
    let queryCookie = "SELECT * FROM aiuroom.person WHERE Universityemail ='" + cookieUser+"'";
    con.query(queryCookie, (err, result) => {
      if (err) console.log(err);
      else res.redirect("/searchStudent");
    });
  }
});
